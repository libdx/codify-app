...
class Users(Resource):
    def get(self, user_id):
        ...
        if user:
            return {'payload': user.to_json(), 'status': 'success'}, 200
        else:
            return {'message': 'Invalid payload', 'status': 'fail'}, 404
...
class UsersList(Resource):
    def get(self):
        ...
        return {'payload': [user.to_json() for user in users], 'status': 'success'}, 200
    ...
    def post(self):
        ...
        if not email:
            return (
                {'message': 'Invalid payload. \'email\' not given', 'status': 'fail'},
                400,
            )
